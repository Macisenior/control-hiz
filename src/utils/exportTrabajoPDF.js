...
export function exportTrabajoPDF(trabajo, registros, materiales, precioHora){
...
doc.text(`HIZ CONTROL - ${trabajo.nombre}`,55,15)
doc.setFontSize(10)
doc.text(`Generado: ${hoy}`,160,15)

doc.setTextColor(0,0,0)

doc.setFontSize(12)

doc.text(`Cliente: ${trabajo.cliente}`,14,40)
doc.text(`Lugar: ${trabajo.lugar}`,14,48)
...
/* =========================
   TABLA HORAS
========================= */
const filasHoras = registros.map(r=>{

const precio = r.precioHora || precioHora || 0
const total = (r.horas || 0) * precio
...
autoTable(doc,{
startY:60,
head:[["Trabajador","Fecha","Horas","Precio","Total"]],
body:filasHoras
})
...
const totalHorasImporte = registros.reduce(
(acc,r)=> acc + ((r.horas || 0) * (r.precioHora || precioHora || 0)),
0
)

doc.text(`Total horas: ${totalHoras} h`,14,doc.lastAutoTable.finalY + 10)

if(precioHora){
doc.text(`Precio hora: ${precioHora} €`,14,doc.lastAutoTable.finalY + 18)
doc.text(`Importe horas: ${totalHorasImporte} €`,14,doc.lastAutoTable.finalY + 26)
}
...
const totalObra = totalHorasImporte + totalMateriales
